-- Drop the 5-param overload that conflicts
DROP FUNCTION IF EXISTS public.score_workbench_priority(text, uuid, integer, text, boolean);

-- Recreate with priority_tag in output
CREATE OR REPLACE FUNCTION public.score_workbench_priority(p_domain text, p_user_id uuid, p_limit integer DEFAULT 10, p_lane text DEFAULT 'all'::text, p_force_content boolean DEFAULT false)
 RETURNS TABLE(id uuid, title text, description text, finding_category text, severity text, target_url text, target_selector text, target_operation text, action_type text, payload jsonb, source_type text, tier integer, base_score integer, severity_bonus integer, aging_bonus integer, gate_malus integer, total_score integer, created_at timestamp with time zone, lane text, priority_tag text)
 LANGUAGE plpgsql
