...
-- Add targeting columns to architect_workbench
ALTER TABLE architect_workbench 
  ADD COLUMN IF NOT EXISTS target_selector TEXT,
  ADD COLUMN IF NOT EXISTS target_operation TEXT DEFAULT 'replace';

COMMENT ON COLUMN architect_workbench.target_selector IS 'CSS selector or CMS field name (e.g. meta_description, h2#avantages, schema_org, header)';
COMMENT ON COLUMN architect_workbench.target_operation IS 'replace, insert_after, append, create, delete_element';

-- Update populate_architect_workbench to fill target_selector and target_operation
...
BEGIN
  ...
  SELECT 
    ...
    -- Infer target_selector from category/fix_type
    CASE
  ...
  IF v_strategic_payload IS NOT NULL THEN
    ...
    IF v_kp IS NOT NULL AND jsonb_typeof(v_kp->'main_keywords') = 'array' THEN
      ...
      SELECT
        ...
        COALESCE(kw->>'target_url', kw->>'page_url', v_domain_url),
    ...
    IF v_kp IS NOT NULL AND jsonb_typeof(v_kp->'quick_wins') = 'array' THEN
      ...
      SELECT
        ...
        COALESCE(qw->>'url', qw->>'page_url', qw->>'target_url', v_domain_url),
        'both'::architect_action_type,
        COALESCE(qw->>'target_field', NULL),
        CASE WHEN qw->>'action' ILIKE '%créer%' OR qw->>'action' ILIKE '%create%' THEN 'create' ELSE 'replace' END,
        qw
    ...
    IF v_kp IS NOT NULL AND jsonb_typeof(v_kp->'content_gaps') = 'array' THEN
      ...
      SELECT
        ...
        COALESCE(cg->>'url', cg->>'suggested_url', v_domain_url),
    ...
    IF v_kp IS NOT NULL AND jsonb_typeof(v_kp->'missing_terms') = 'array' THEN
      ...
      SELECT
        ...
        COALESCE(mt->>'url', mt->>'page_url', v_domain_url),
        'both'::architect_action_type,
        COALESCE(mt->>'target_element', 'content'),
        'append',
        mt
    ...
    IF v_pc IS NOT NULL AND jsonb_typeof(v_pc->'missing_pages') = 'array' THEN
      ...
      SELECT
        ...
        COALESCE(mp->>'suggested_url', v_domain_url || '/' || COALESCE(mp->>'slug', 'new-page')),
        'content'::architect_action_type,
        NULL,
        'create',
        mp
    ...
    IF v_pc IS NOT NULL AND jsonb_typeof(v_pc->'content_upgrades') = 'array' THEN
      ...
      SELECT
        ...
        COALESCE(cu->>'url', cu->>'page_url', cu->>'page', v_domain_url),
        'both'::architect_action_type,
        'content',
        'replace',
        cu
    ...
    IF v_kp IS NOT NULL AND jsonb_typeof(v_kp->'serp_recommendations') = 'array' THEN
      ...
      SELECT
        ...
        COALESCE(sr->>'url', sr->>'target_url', v_domain_url),
        'both'::architect_action_type,
        'replace',
        sr
