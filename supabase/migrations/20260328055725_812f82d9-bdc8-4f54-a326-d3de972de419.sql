...
-- Function: score_workbench_priority
-- Returns workbench items with computed priority scores based on the 11-tier pyramid
-- Gates: if tech audit score < 70, tiers 5+ receive a -500 malus
...
DECLARE
  ...
  v_strategic_score NUMERIC := 100;
BEGIN
  -- Get latest audit scores for gate logic
  BEGIN
    SELECT COALESCE(
  ...
  WITH scored AS (
    SELECT
      ...
      CASE
        ...
        ELSE 6  -- default to mid-tier
      END AS computed_tier,
      -- Base score by tier (higher tier number = lower priority)
      CASE
      ...
      -- Aging bonus: +10 per day, capped at 100
    ...
    WHERE w.domain = p_domain
      ...
      AND w.status IN ('new', 'in_progress')
  ...
  SELECT
    ...
    -- Gate malus: if tech score < 70, tiers 5+ get -500
