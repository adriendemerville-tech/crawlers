...
-- 1. Index composite pour tri par URL puis priorité
CREATE INDEX IF NOT EXISTS idx_architect_workbench_url_severity 
ON public.architect_workbench (domain, target_url, severity);

-- 2. Mettre à jour la fonction populate_architect_workbench pour mieux remplir target_url
...
BEGIN
  ...
  IF v_strategic_payload IS NOT NULL THEN
    ...
    IF v_kp IS NOT NULL AND jsonb_typeof(v_kp->'main_keywords') = 'array' THEN
      INSERT INTO architect_workbench (domain, tracked_site_id, user_id, source_type, source_function, source_record_id, finding_category, severity, title, description, target_url, action_type, payload)
      SELECT
  ...
  -- ── 5. Backfill NULL target_urls with domain URL ──
