...
-- ═══ A+C+D: Add dual-lane config columns to autopilot_configs ═══
ALTER TABLE public.autopilot_configs 
  ADD COLUMN IF NOT EXISTS force_content_cycle boolean DEFAULT false,
  ADD COLUMN IF NOT EXISTS content_budget_pct integer DEFAULT 30,
  ADD COLUMN IF NOT EXISTS gate_threshold_low integer DEFAULT 50,
  ADD COLUMN IF NOT EXISTS gate_threshold_high integer DEFAULT 70;

-- ═══ B+C: Refactor score_workbench_priority with progressive gate + dual-lane support ═══
...
BEGIN
  ...
  WITH scored AS (
    SELECT
      ...
      -- Lane assignment (tech vs content)
      CASE 
  ...
  SELECT
    ...
    -- Progressive gate (Option C): 
    -- tier 5-6 blocked only if tech_score < gate_low (50)
    -- tier 7+ blocked only if tech_score < gate_high (70)
    -- force_content bypasses gate entirely (Option D)
    CASE 
