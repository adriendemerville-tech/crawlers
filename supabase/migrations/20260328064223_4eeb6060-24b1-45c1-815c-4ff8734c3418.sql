...
CREATE OR REPLACE FUNCTION public.score_workbench_priority(p_domain text, p_user_id uuid, p_limit integer DEFAULT 10)
 RETURNS TABLE(id uuid, title text, description text, finding_category text, severity text, target_url text, target_selector text, target_operation text, action_type text, payload jsonb, source_type text, tier integer, base_score integer, severity_bonus integer, aging_bonus integer, gate_malus integer, total_score integer, created_at timestamp with time zone)
 LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT
    ...
    s.target_selector, s.target_operation,
