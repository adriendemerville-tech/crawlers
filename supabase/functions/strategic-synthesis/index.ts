...
function formatToolsDataToMarkdown(toolsData: any): string {
  const lines: string[] = [];
  if (toolsData?.crawlers) { const c = toolsData.crawlers; lines.push('## CRAWLERS'); if (c.overallScore != null) lines.push(`Score: ${c.overallScore}/100`); if (c.bots) for (const b of c.bots) if (b.name) lines.push(`- ${b.name}: ${b.isAllowed ? 'autorise' : 'bloque'}`); if (c.recommendations) lines.push(`Recs: ${c.recommendations.slice(0, 5).join('; ')}`); }
