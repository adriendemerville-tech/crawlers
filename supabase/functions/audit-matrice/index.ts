...
const BALISE_KEYWORDS = [
  'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'title', 'meta description', 'meta_description',
  'canonical', 'alt', 'viewport', 'og:', 'open graph', 'twitter:', 'hreflang',
  'meta desc', 'balise', 'tag', 'author', 'auteur', 'img', 'image alt',
  'favicon', 'lang', 'charset', 'meta robot', 'noindex', 'nofollow',
]

const STRUCTURED_DATA_KEYWORDS = [
  'schema', 'json-ld', 'jsonld', 'json ld', 'structured data', 'données structurées',
  'robots.txt', 'robots txt', 'sitemap', 'llms.txt', 'llms txt',
  'organization', 'faqpage', 'breadcrumb', 'article', 'product', 'review',
  'person', 'website', 'websi', 'howto', 'local business', '@type', '@graph',
  'sameas', 'same_as',
]

const PERFORMANCE_KEYWORDS = [
  'lcp', 'fcp', 'cls', 'tbt', 'tti', 'speed index', 'performance',
  'vitesse', 'temps de chargement', 'loading', 'core web vitals', 'cwv',
  'pagespeed', 'lighthouse',
]

const SECURITY_KEYWORDS = [
  'https', 'hsts', 'ssl', 'tls', 'safe browsing', 'sécurité', 'security',
  'certificat', 'certificate',
]

const COMBINED_KEYWORDS = [
  'score global', 'score technique', 'score seo', 'score sémantique',
  'semantic score', 'technical score', 'ai ready', 'ia ready',
  'maillage', 'internal link', 'liens internes', 'external link',
  'liens externes', 'e-e-a-t', 'eeat', 'densité', 'data density',
  'fraîcheur', 'freshness', 'contenu', 'content quality',
  'social', 'linkedin', 'case study', 'étude de cas',
  'faq', 'tldr', 'tableau', 'table', 'liste', 'list',
]
...
function detectItemType(prompt: string, llmName?: string): ItemType {
  ...
  // If it has a custom LLM and looks like a sentence/question → prompt
  ...
  // Check keywords
  for (const kw of BALISE_KEYWORDS) {
    if (lower.includes(kw)) return 'balise'
  }
  for (const kw of STRUCTURED_DATA_KEYWORDS) {
    if (lower.includes(kw)) return 'structured_data'
  }
  for (const kw of PERFORMANCE_KEYWORDS) {
    if (lower.includes(kw)) return 'performance'
  }
  for (const kw of SECURITY_KEYWORDS) {
    if (lower.includes(kw)) return 'security'
  }
  for (const kw of COMBINED_KEYWORDS) {
    if (lower.includes(kw)) return 'metric_combinee'
  }

  // If it's a long sentence, treat as prompt
