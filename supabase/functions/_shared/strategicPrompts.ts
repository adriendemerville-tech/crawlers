/**
 * System prompts and user prompt builder for strategic audit.
 * Extracted from audit-strategique-ia to reduce monolith size.
 */

import {
  type ToolsData, type MarketData, type RankingOverview, type EEATSignals,
  type FounderInfo, type FacebookPageInfo,
} from './textUtils.ts';
...
- missing_terms: MIN 3 termes clés que les concurrents SERP utilisent mais que cette page n'utilise pas.
- semantic_density: compare la richesse sémantique de la page vs les 3 premiers concurrents SERP.
- serp_recommendations: MIN 3 actions concrètes et actionnables.
- alternative_strategy: UNIQUEMENT si position très défavorable (>50). Sinon null.
...
- serp_recommendations: MIN 3 actions concrètes et actionnables.
- alternative_strategy: UNIQUEMENT si position très défavorable (>50). Sinon null.
- client_targets: OBLIGATOIRE. primary: 1-2 cibles (confidence>0.7). secondary: 1-2. untapped: exactement 2.
- JSON pur, sans virgules traînantes`;
}

// ==================== RECOMMENDATION HELPERS ====================

export function generateStrategicPromptSummary(title: string, description: string, priority: string): string {
