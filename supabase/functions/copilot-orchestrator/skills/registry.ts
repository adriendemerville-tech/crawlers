...
const detect_content_cannibalization: SkillDefinition = {
  ...
  handler: async (input, ctx) => {
    ...
    const res = await computeCannibalization(ctx.supabase, {
      domain,
      threshold: Number(input.threshold ?? 0.45),
      pathPrefix: String(input.path_prefix ?? ''),
    });
    ...
    return {
      ...
      data: {
        ...
        analyzed_pages: res.analyzed_pages,
        clusters_count: res.clusters_count,
        redundant_pages: res.redundant_pages,
        clusters: res.clusters,
        report_markdown: res.report_markdown,
        next_step: res.clusters_count > 0
