...
Deno.serve(handleRequest(async (req) => {
  ...
  for (const site of sites) {
    try {
        ...
        `https://www.googleapis.com/webmasters/v3/sites/${encodeURIComponent(siteUrl)}/searchAnalytics/query`,
        {
          ...
          body: JSON.stringify({
            ...
            dimensionFilterGroups: [{
              filters: [{ dimension: 'country', expression: 'fra' }]
            }],
          }),
      ...
      if (!gscRes.ok) {
        console.warn(`[fetch-gsc-daily] GSC API error for ${cleanDomain}: ${gscRes.status}`)
        continue
      }
      ...
      for (let i = 0; i < insertRows.length; i += 100) {
        ...
        const { error } = await supabase.from('gsc_daily_positions').upsert(batch, {
          onConflict: 'tracked_site_id,query,date_val',
        })
        if (error) console.error(`[fetch-gsc-daily] Upsert error for ${cleanDomain}:`, error)
        else totalInserted += batch.length
      }

      console.log(`[fetch-gsc-daily] ✅ ${cleanDomain}: ${rows.length} queries stored for ${dateStr}`)
    } catch (e) {
      console.error(`[fetch-gsc-daily] Error for ${site.domain}:`, e)
    }
  }
