...
BEGIN
  ...
  -- Mémoire : récompense GSC par cluster (identifiants texte → uuid, sinon la
  -- jointure échouait et la réinjection n'avait jamais lieu)
  cluster_reward AS (
    SELECT goal_cluster_id::uuid AS cluster_id, AVG(reward_signal) AS avg_reward
    FROM pericles_decision_log
    WHERE domain = p_domain AND goal_cluster_id IS NOT NULL
      AND goal_cluster_id ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
      AND measured_at IS NOT NULL AND reward_signal IS NOT NULL
      AND measured_at > v_now - interval '180 days'
    GROUP BY goal_cluster_id
  ),
  -- Mémoire : récompense IA (GEO) par cluster
  cluster_geo_reward AS (
    SELECT goal_cluster_id::uuid AS cluster_id, AVG(geo_reward_signal) AS avg_geo_reward
    FROM pericles_decision_log
    WHERE domain = p_domain AND goal_cluster_id IS NOT NULL
      AND goal_cluster_id ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
  ...
  cluster_failures AS (
    SELECT cluster_id, SUM(n) AS fails FROM (
      SELECT goal_cluster_id::uuid AS cluster_id, COUNT(*) AS n
      FROM pericles_decision_log
      WHERE domain = p_domain AND goal_cluster_id IS NOT NULL
        AND goal_cluster_id ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
