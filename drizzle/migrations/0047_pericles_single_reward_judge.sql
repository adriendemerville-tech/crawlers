-- Juge unique de la recompense : Parmenion et Pericles relisent la ligne avant
-- d'ecrire. Une mesure de moins de 7 jours est reutilisee, sinon elle est refaite.
ALTER TABLE public.pericles_decision_log
  ADD COLUMN IF NOT EXISTS reward_measured_by text;
...
DECLARE
  ...
  v_scope text;
  v_geo numeric;
  v_lock_measured_at timestamptz;
  v_reused integer := 0;
BEGIN
  FOR v_rec IN
    ...
    WHERE d.status = 'completed'
      AND (d.measured_at IS NULL OR d.measured_at < now() - interval '7 days')
  ...
  LOOP
    -- Juge unique : on relit la ligne (verrou) avant d'ecrire. Une mesure de
    -- moins de 7 jours est reutilisee telle quelle, quel que soit l'appelant.
    SELECT measured_at INTO v_lock_measured_at
    FROM pericles_decision_log WHERE id = v_rec.id FOR UPDATE;
    IF v_lock_measured_at IS NOT NULL
       AND v_lock_measured_at > now() - interval '7 days' THEN
      v_reused := v_reused + 1;
      CONTINUE;
    END IF;

    v_urls := public.pericles_decision_urls(v_rec.id);
    v_queries := public.pericles_decision_queries(v_rec.id, v_rec.domain);

    SELECT COUNT(DISTINCT g.query) INTO v_kw_seen
    FROM gsc_daily_positions g
    WHERE g.domain = v_rec.domain
      AND COALESCE(array_length(v_queries, 1), 0) > 0
      AND lower(g.query) = ANY(v_queries)
    ...
    WHERE domain = v_rec.domain
      AND (v_scope = 'domain' OR lower(query) = ANY(v_queries))
    ...
    UPDATE pericles_decision_log SET
      ...
      measured_keyword_count = COALESCE(v_kw_seen, 0),
      ...
      calibration_note = format('Mesure GSC %s j (%s, %s requêtes, %s pages) : clics %s%%, position %s, récompense %s',
        p_window_days, v_scope, COALESCE(v_kw_seen, 0), COALESCE(array_length(v_urls, 1), 0),
        ROUND(v_clicks_delta, 1), ROUND(v_pos_delta, 2), v_reward),
      measured_at = now(),
      reward_measured_by = 'pericles',
      updated_at = now()
    WHERE id = v_rec.id;
    ...
    v_geo := public.pericles_geo_reward(v_rec.id);
