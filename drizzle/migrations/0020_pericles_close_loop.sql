-- ═══ Périclès : fermeture de la boucle récursive ═══
-- 1) Santé de récompense par domaine (lue par le Contrôleur / seuil de pause)
CREATE OR REPLACE FUNCTION public.pericles_reward_health(p_domain text)
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.pericles_reward_health(text) TO authenticated, service_role;

-- 2) Scoring spiralé v3 : biais de récompense + exclusion des échecs répétés
...
GRANT EXECUTE ON FUNCTION public.score_spiral_priority(text, uuid, integer, text, boolean) TO authenticated, service_role;

COMMENT ON FUNCTION public.score_spiral_priority(text, uuid, integer, text, boolean) IS 'Scoring spiralé v3 : réinjecte le signal de récompense mesuré à T+30 (par cluster, repli domaine), pénalise et exclut les échecs répétés (validate_attempts >= 3, clusters >= 4 échecs sur 90j sauf urgence).';