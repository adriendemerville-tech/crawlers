-- Résolution des pages réellement visées par une décision : constats rattachés,
-- puis URLs présentes dans la charge utile (affected_urls / target_urls / target_url).
...
BEGIN
  ...
  LOOP
    ...
    WHERE ku.domain = v_rec.domain
      AND (
        (array_length(v_urls, 1) > 0 AND rtrim(ku.target_url, '/') = ANY(v_urls))
        OR (
          COALESCE(array_length(v_urls, 1), 0) = 0
    ...
    UPDATE pericles_decision_log SET
      ...
      calibration_note = format('Mesure GSC %s j (%s, %s mots-clés, %s pages) : clics %s%%, position %s, récompense %s',
