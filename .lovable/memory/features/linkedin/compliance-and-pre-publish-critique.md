...
- **Bloc GEO question/réponse (obligatoire)** : chaque post contient exactement 1 question directe rédigée comme un prompt d'utilisateur d'IA (Pourquoi, Comment, Combien, Qui, Quand, Quoi), seule sur sa ligne, suivie immédiatement d'une réponse autoportante de 1 à 2 phrases nommant Crawlers ou le module. Vérifié en déterministe par `GEO_QUESTION_RE` (check `geo_question` valide que le nombre de blocs est égal à 1), exigé dans le `systemPrompt`, le `userPrompt` (étape 3 de la structure) et la `CRITIQUE_SYSTEM`. L'interrogatif doit varier d'un post à l'autre.
