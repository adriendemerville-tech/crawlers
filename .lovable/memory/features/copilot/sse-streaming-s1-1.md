...
- `aiGatewayCallStream` (`_shared/aiGatewayFetch.ts`) : même cascade primary→fallback qu'`aiGatewayCall`, mais `stream: true` + parseur SSE côté client. Ultime recours = synthèse SSE monobloc depuis appel non-streaming (uniformise le parseur).
...
- Fallback vers `sendMessage()` classique si l'ouverture échoue.
