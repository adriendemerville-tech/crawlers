...
- **Toggle on/off** : `tracked_sites.current_config.dictadevi_context_enabled` (défaut `true`).
  - **UI : Admin → Parménion (panel cible Dictadevi)** — bouton `Contexte:ON / Contexte` (icône Database) à côté de "Prio contenu", visible uniquement si `targetDomain` contient `dictadevi`.
  - Si `false`, le contexte n'est pas fetché et aucune règle n'est injectée dans le prompt Writer.
