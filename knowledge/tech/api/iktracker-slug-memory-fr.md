...
CREATE TABLE public.iktracker_slug_memory (
  ...
  status            TEXT NOT NULL CHECK (status IN
                      ('published','blacklisted','skipped','duplicate','error','trashed')),
  reason            TEXT NULL,
...
| `trashed` | Soft-delete corbeille IKtracker (HTTP 409 `slug_in_trash`) | **Bloque définitivement** — restauration admin uniquement, jamais de retry auto |
...
> **API IKtracker — 4 statuts publics** : `draft`, `published`, `archived`, `deleted`. Seul `published` est listé par `GET /posts`. Les `archived` restent accessibles via URL directe ; les `deleted` sont en corbeille (soft-delete via `DELETE /posts/:slug`, purge dure via `?hard=true` à éviter). Côté Crawlers, on ne voit que les conséquences API : `409 slug_in_trash` lorsqu'on tente de recréer un slug en corbeille → mappé en `trashed` ci-dessus.
...
| 409 | `error = slug_in_trash` | `trashed` |
| 409 | autre | `duplicate` |
