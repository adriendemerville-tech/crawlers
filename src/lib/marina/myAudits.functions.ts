  ...
  .handler(async ({ context }) => {
    ...
    return (data || []).map((j: any) => {
      ...
      return {
        ...
        multipage: Boolean(payload.urls?.length || payload.multipage || payload.batch_id),
        batchId: (payload.batch_id || null) as string | null,
        batchSize: (payload.batch_size ?? null) as number | null,
