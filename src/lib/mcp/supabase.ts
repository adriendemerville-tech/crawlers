...
function supabasePublishableKey(): string {
  ...
  if (keyset) {
    try {
      ...
      // dictionnaire malformé → on retombe sur les noms historiques
    }
  }
  const legacy = configuredEnv(["SUPABASE_ANON_KEY", "VITE_SUPABASE_ANON_KEY"]);
  if (legacy) return legacy;
  throw new Error("Clé publique Supabase requise");
}

/** Client portant le jeton vérifié de l'appelant : RLS s'applique comme cet utilisateur. */
...
/** Appel de l'API développeurs (/v1/*) au nom de l'utilisateur MCP. */
export async function crawlersApi(
  ctx: ToolContext,
  path: string,
  init: { method?: string; body?: unknown; tool?: string; idempotencyKey?: string } = {},
