...
// SEO metadata for this page - forced via useEffect to override index.html defaults
const SEO_META = {
  title: "Alternative Semrush Gratuite : Le Comparatif 2026 (SEO & GEO)",
  description: "Audit SEO & GEO gratuit vs Semrush. Pourquoi payer 130$ pour un audit technique ? Découvrez l'alternative française optimisée pour l'IA et ChatGPT.",
  ogTitle: "Alternative Semrush Gratuite : Le Comparatif 2026",
  canonical: `${SITE_URL}/comparatif-crawlers-semrush`
};

/**
 * Force les métadonnées dans le <head> via manipulation directe du DOM
 * Garantit que les balises écrasent celles par défaut (index.html) lors de la navigation CSR
 */
function forceMetaTags() {
  ...
  if (metaDesc) {
    metaDesc.content = SEO_META.description;
  } else {
  ...
  if (ogTitleMeta) {
    ogTitleMeta.content = SEO_META.ogTitle;
  } else {
  ...
  if (ogDescMeta) {
    ogDescMeta.content = SEO_META.description;
  } else {
  ...
  if (ogUrlMeta) {
    ogUrlMeta.content = SEO_META.canonical;
  } else {
  ...
  if (twitterTitle) {
    twitterTitle.content = SEO_META.ogTitle;
  } else {
  ...
  if (twitterDesc) {
    twitterDesc.content = SEO_META.description;
  } else {
