      ...
      <DialogContent className="sm:max-w-[1200px] max-h-[95vh] overflow-y-auto">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <div className="flex-1 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-2 pt-1">
                ...
                <span className="flex items-center gap-1.5 text-xs font-medium text-foreground"><Zap className="h-3.5 w-3.5 text-violet-500" />{language === 'fr' ? 'Indice IAS' : language === 'es' ? 'Índice IAS' : 'SAI Index'}</span>
                <span className="flex items-center gap-1.5 text-xs font-medium text-foreground"><Network className="h-3.5 w-3.5 text-violet-500" />{language === 'fr' ? 'Architecte de Cocon & GEO' : language === 'es' ? 'Arquitecto Cocon & GEO' : 'Cocoon Architect & GEO'}</span>
                <span className="flex items-center gap-1.5 text-xs font-medium text-foreground"><FileText className="h-3.5 w-3.5 text-violet-500" />{language === 'fr' ? 'Content Architect : 80 pages créés inclus' : language === 'es' ? 'Content Architect: 80 páginas creadas incluidas' : 'Content Architect: 80 pages created included'}</span>
              </div>
            </div>
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            <div className="flex flex-col items-center gap-2 shrink-0">
              <p className="text-2xl font-bold text-foreground">79€<span className="text-sm font-normal text-muted-foreground">/{language === 'fr' ? 'mois' : language === 'es' ? 'mes' : 'mo'}</span></p>
