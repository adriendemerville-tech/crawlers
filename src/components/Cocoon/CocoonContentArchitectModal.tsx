          ...
          <div className="w-[340px] shrink-0 border-r border-white/10 flex flex-col">
            ...
            <div className="shrink-0 p-4 border-t border-white/10 space-y-2">
              <Button onClick={handleGenerate} disabled={loading || !keyword || (!directory && !slug)} className="w-full bg-[#fbbf24] hover:bg-[#f59e0b] text-[#0f0a1e] font-semibold h-9 text-xs">
                {loading ? <><Loader2 className="w-3.5 h-3.5 animate-spin mr-2" />{t3(language, 'Génération…', 'Generating…', 'Generando…')}</> : <><Send className="w-3.5 h-3.5 mr-2" />{t3(language, 'Générer le contenu', 'Generate content', 'Generar contenido')}</>}
              </Button>
              {result && (
