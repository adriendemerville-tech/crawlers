...
export function Header() {
  ...
  return (
    <>
    <header className={`fixed top-3 left-1/2 z-50 w-[calc(100%-1.5rem)] max-w-6xl -translate-x-1/2 rounded-2xl border border-border/50 bg-card/70 backdrop-blur-xl shadow-lg shadow-black/5 transition-transform duration-300 ease-in-out ${headerHidden ? '-translate-y-[calc(100%+1rem)]' : ''}`} role="banner">
          ...
          {isAuditExpertPage ? (
            ...
            <>
               <a href="/" className="relative flex flex-col items-start" aria-label="Crawlers.fr - Accueil">
                ...
                <span className="text-lg font-display text-[#7c3aed]" style={{ fontWeight: 900 }}>
                  {isProfilePage ? (
            ...
            user ? (
              ...
              <div className="flex items-center gap-1.5">
                <Link to="/auth" aria-label={t.login}>
                  <Button variant="ghost" size="sm" className="gap-2 text-foreground hover:bg-muted/50" aria-label={t.login}>
