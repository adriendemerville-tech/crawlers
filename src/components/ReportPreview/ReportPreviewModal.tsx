...
export function ReportPreviewModal({
  ...
  }, [isOpen, user, hasAnyData, url, effectiveType, language, data, saveReport]);
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      if (!pdfData) throw new Error('No report data available');
  ...
  const handleDownloadCSV = async () => {
    if (effectiveType !== 'site_crawl' || !siteCrawlData) return;

    const headers = ['URL', 'Path', 'Score SEO', 'HTTP Status', 'Title', 'Word Count', 'Noindex', 'Issues'];
    const rows = siteCrawlData.pages.map((page) => [
      page.url,
      page.path,
      page.seo_score ?? '',
      page.http_status ?? '',
    ...
    link.download = getReportFilename(siteCrawlData.domain, 'crawl', 'csv');
  ...
  const handleCopyLink = async () => {
    ...
    try {
      ...
      if (!linkToCopy) {
        ...
        if (linkToCopy) setShareUrl(linkToCopy);
      }
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (!linkToShare) {
        ...
        if (linkToShare) setShareUrl(linkToShare);
      }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent
        className="max-w-6xl max-h-[95vh] overflow-hidden flex flex-col p-0 [&>button]:hidden"
        ...
        <div className="flex items-center justify-between px-4 md:px-6 py-3 md:py-4 border-b border-border bg-card w-full shrink-0">
          {!isMobile && (
            <h2 className="text-lg font-semibold shrink-0">
              {effectiveType === 'site_crawl' ? `${t.crawlTitle} — ${siteCrawlData?.domain || url}` : t.title}
            </h2>
          )}

          <div className="flex items-center gap-2 md:gap-3 ml-auto">
            {effectiveType === 'site_crawl' && (
              <Button
