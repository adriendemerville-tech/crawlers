      ...
      {!(isOpen && felixExpanded) && !cocoonExpanded && (
        <>
          <button
            ...
            className={`fixed bottom-5 z-[110] h-[3.15rem] w-[3.15rem] rounded-full flex items-center justify-center transition-all duration-300 border border-foreground/30 dark:border-white/80 bg-background/90 shadow-lg backdrop-blur hover:bg-background hover:scale-105 focus:outline-hidden focus-visible:ring-2 focus-visible:ring-primary overflow-hidden ${showBounce ? 'animate-felix-bounce' : ''}`}
            style={{ right: 'max(0.25rem, calc((100vw - 72rem) / 2 - 3.5rem))' }}
            aria-label={isOpen ? 'Fermer le chat' : 'Ouvrir le chat support'}
          >
            <CrawlersLogo size={56} className="transition-opacity duration-300" />
          </button>
          {/* Notification Badge — only for real unread support messages */}
